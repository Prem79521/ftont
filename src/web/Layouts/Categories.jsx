...
const Categories = () => {
    return (
        <section className="hidden sm:block mt-10 mb-4 min-w-full px-12 py-1 overflow-hidden glass">
            ...
            <div className="flex items-center justify-between mt-4">
                ...
                {catNav.map((item, i) => (
                    <Link to={`/products?category=${item.name}`} className="flex flex-col gap-1 items-center p-2 group" key={i}>
                        ...
                        <span className="text-sm font-medium group-hover:text-primary-blue">{item.name}</span>
                    </Link>
                ))}
